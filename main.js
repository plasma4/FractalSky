...
if (!window.WebAssembly || !WebAssembly.instantiateStreaming) {
  ...
  help.innerHTML =
    'WebAssembly appears to be disabled currenly, so unfortunately, the program cannot function. Perhaps you are using a strict browser or have disabled it?<br><button onclick="help.removeAttribute(\'style\')" id="infoClose">Close</button>';
  throw new Error("WebAssembly is not supported. ):");
} else if (!useSharedWebWorkers) {
  document.getElementById("noteContent").innerHTML +=
    'Unfortunately, this fractal viewer may only use a single core due to headers not enabling SharedArrayBuffer. Try <a href="https://plasma4.org/projects/FractalSky/">this link</a> for a multicore version. ' +
...
window.addEventListener("load", function () {
  if (!navigator.serviceWorker) {
    console.warn("Service workers not supported in this context.");
    return;
  }

  const needsHeaders =
    !crossOriginIsolated && window.location.protocol !== "file:";
...
function oklabToSRGB(oklab_array) {
  ...
  var lmsOklab = multiplyMatrixVector(M2_INVERSE_OKLAB_TO_LMS_NON_LINEAR, [
    L,
    a,
    b,
  ]);
...
function generateSRGBLinearLUT() {
  ...
  for (var i = 0; i < 256; i++) {
    ...
    lut[i] =
      srgb_norm > 0.04045
        ? Math.pow((srgb_norm + 0.055) / 1.055, 2.4)
        : srgb_norm / 12.92;
  }
  return lut;
}
...
function generateLinearSRGBLUT() {
  ...
  for (var i = 0; i < 256; i++) {
    ...
    var srgb_val =
      linear_norm > 0.0031308
        ? 1.055 * Math.pow(linear_norm, 1 / 2.4) - 0.055
        : 12.92 * linear_norm;
...
function initializeColorScience(
  ...
  for (var originalValue = 0; originalValue < 256; originalValue++) {
    ...
    for (var shadeAmount = 0; shadeAmount < numShades; shadeAmount++) {
      ...
      var finalSrgbValue =
        linearToSrgbLUT[
          Math.round(Math.max(0, Math.min(1, shadedLinearValue)) * 255)
        ];
...
const shadingNames = ["Default", "Shadow", "Inverted Shadow", "Stripes"];

var urlParameters = parseGracefulUrlParams(location);
const workerCount = useSharedWebWorkers
  ? urlParameters.get("workers")
    ? +urlParameters.get("workers")
    : navigator.hardwareConcurrency
  : 1;
document.getElementById("hardwareWorkers").textContent =
  navigator.hardwareConcurrency +
  (navigator.hardwareConcurrency === 1 ? " worker" : " workers") +
  (workerCount !== navigator.hardwareConcurrency
    ? " (overridden to " + workerCount + ")"
    : "");
...
if (isSafari) {
  notice.textContent =
    "IMPORTANT: Safari is significantly slower for this program than other browsers, and misses out on a few minor features. Try using a browser like Firefox or Chrome to get the best speed and performance.";
}
...
function updateProgressLine(currentPixel) {
  ...
  line.style.top =
    (Math.floor(currentPixel / w) / devicePixelRatio).toFixed(1) + "px";
...
FakeWorker.prototype.setupWorker = function () {
  ...
  WebAssembly.instantiateStreaming(fetch(this.wasmFileName), {
    // Use the provided WASM file name
    env: {
      memory: this.memory,
    },
  })
    .then((result) => {
      ...
      scheduleTask(() => this._postMessageFromWorker(-2));
    })
    .catch((e) => {
...
const palettes = [
  [
    0x0a0aa0, 0x3232ff, 0x00c8ff, 0x00b43c, 0xdcb428, 0x7d643c, 0xdcc8c8,
    0xc864aa, 0x820a8c, 0x7d00b9, 0x375ff5, 0x14a0e6, 0x5fe1dc, 0x8ce1c8,
    0x9bc87d, 0xf08750, 0xe650aa, 0xa564f0,
  ],
  [
    0xb65741, 0x302f33, 0xb85740, 0xe7eaeb, 0xb5a551, 0x181917, 0xb05b50,
    0xc9c5ca, 0xb5a64b, 0x444647, 0xb9a547, 0xeae5ea, 0xbb5d3f, 0x161516,
    0xb5a852, 0xc4c6c4, 0xb15b4b, 0x434043, 0xb05e50, 0xe5e9eb, 0xb9a64b,
    0x181b19, 0xba5c44, 0xd2d8d6, 0xb9a64f, 0x454445, 0xb6a04c, 0xe8e8e6,
    0xb3a557, 0x191619, 0xb4a55a, 0xcac4c9, 0xaf5a4e, 0x36353a, 0xb9a954,
    0xe6eaea, 0xb1564e, 0x141818, 0xb5a857, 0xc5c1c5, 0xafa35d, 0x454849,
    0xb4a459, 0xe8ebe9, 0xc3b397, 0x141b14, 0xafa85a, 0xd8d6d7, 0xb3a35c,
    0x43443f, 0xb3c15c, 0xe6e5e6, 0x9cd083, 0x17161b, 0x5abb50, 0xc3c4c2,
    0xd2cb4c, 0x322f32,
  ], // this was NOT manually done; don't worry
  [
    0xc8c8ff, 0x4cdbff, 0x0692d7, 0x122c91, 0x371663, 0x600d39, 0x8a030f,
    0xbf2600, 0xfb6200, 0xffc9a6, 0x000000,
  ],
];
...
function setupWebWorkers(amount) {
  for (let i = 0; i < amount; i++) {
    ...
    try {
      ...
      help.style.display = "unset";
      help.innerHTML =
        'Unfortunately, an error occured while creating the memory. This may be because of default memory limitations; try adding ?maxMB=4096 to the start of the URL (no commas in the number), or check the console if that doesn\'t work.<br><button onclick="help.removeAttribute(\'style\')" id="infoClose">Close</button>';
      throw e;
    }
    ...
    worker.onmessage = function (e) {
      ...
      if (++workersDone === workerCount) {
        ...
        if (data == null) {
          ...
          var lowestProgress =
            progressValues.length > 0 ? Math.min(...progressValues) : -1;
          ...
          if (!useSharedWebWorkers) {
            ...
            // Real workers are on a separate thread.
            // We don't need to yield the main thread logic, just update the display.
...
function resizeHandler() {
  ...
  canvas.style.width =
    ...
    hidden.style.width =
      Math.ceil(innerWidth) + "px";
  canvas.style.height =
    previous.style.height =
    hidden.style.height =
      Math.ceil(innerHeight) + "px";
...
function expandMemory(finalByte) {
  try {
    ...
    help.style.display = "unset";
    help.innerHTML =
      'Unfortunately, an error occured while adding more memory. This may be because of default memory limitations; try adding ?maxMB=4096 to the start of the URL (no commas in the number), or check the console if that doesn\'t work. Press Close, copy the current fractal position to save if necessary.<br><button onclick="help.removeAttribute(\'style\')" id="infoClose">Close</button>';
    throw e;
  }
}
...
document.addEventListener(
  ...
  function (e) {
    ...
    if (pinching) {
      ...
      pinchDist =
        Math.sqrt(xPinch * xPinch + yPinch * yPinch) * devicePixelRatio;
    } else if (length === 1) {
      touchDown = true;
...
document.addEventListener(
  ...
  function (e) {
    ...
    if (pinching) {
      ...
      var newDist =
        Math.sqrt(xPinch * xPinch + yPinch * yPinch) *
        devicePixelRatio *
        quality;
...
function update() {
  ...
  if (pixelBreakdown) {
    // Useful for debugging and getting info about a specific pixel
      ...
      percent.textContent =
        ...
        (colorBytes[4 * p + 3] === 0
          ...
          : colorBytes[4 + p] +
            ...
            colorBytes[4 * p + 2]) +
        "\r\nShading amount: " +
        (shade === 0 ? "None" : shade.toFixed(6));
    } else if (currentX >= 0 && currentY >= 0) {
      percent.textContent =
        "Cursor location: (" + currentX + ", " + currentY + ")";
    } else {
  ...
  if (shadingEffect !== newShading) {
    if (
      (shadingEffect === 1 && newShading === 2) ||
      (shadingEffect === 2 && newShading === 1)
    ) {
      requestRender();
  ...
  if (unfinished || rehandle) {
    ...
    for (var t = 0; t < workerCount; t++) {
      messageWebWorker(t, [
        1,
        juliaMode ? -fractalType : fractalType,
        w,
        h,
        panX,
        panY,
        ...
        juliaX,
        juliaY,
      ]); // Message the parameters to be passed into each worker.
    }
  } else {
    line.removeAttribute("style");
    if (flowRate !== 0) {
...
function completeRender(animatedMode) {
  // animatedMode must be true in order for this to work; not some value that equals true.
    ...
    if (unfinished) {
      ...
      if (!pixelBreakdown)
        percent.textContent =
          (100 * ratio).toFixed(2) +
          "% finished (Taking " +
          timeToString(mainDiff) +
          ")\r\nEstimated time left: " +
          timeToString(timeGuess, true) +
          "\r\nPerformance: " +
          (totalCost / workerCount / wantedFPS / 1000).toFixed(2) +
          "/thread (for " +
          workerCount +
          " " +
          (workerCount === 1 ? "thread" : "threads") +
          ").\nRendering took " +
          (performance.now() - imgTime).toFixed(2) +
          "ms (running at " +
          fps.toFixed(1) +
          "fps)";
      if (pixelDiff >= 768) {
      ...
      line.style.top =
        (
          Math.floor(highestProgress / (w * quality)) / devicePixelRatio
        ).toFixed(1) + "px";
      line.style.backgroundColor = color;
    } else {
      ...
      if (!pixelBreakdown) {
        if (flowRate === 0 || mainDiff < 2000) {
          percent.textContent =
            "Took " +
            timeToString(mainDiff, true) +
            (quality === 1
              ? "."
              : " (with the quality factor set to " + quality + ").");
...
function timeToString(ms, fullLength) {
  ...
  if (fullLength) {
    if (ms < 1000) {
      ...
      return (
        (m === 1 ? "1 minute and " : m + " minutes and ") +
    ...
    if (ms < 1000) {
      ...
      return (
        Math.floor(ms / 60000) + "m " + Math.floor((ms * 0.001) % 60) + "s"
      );
    } else {
      return (
...
function resetLocation() {
  ...
  panY =
    (-h * 0.5 - 0.5) * zoom +
    1e-5 +
    (fractalType === 7 ? -0.4 : fractalType === 8 ? 0.25 : 0);
  clearBack();
}
...
function decodePalette(colors) {
  ...
  for (var i = 0; i < rgbLength; i++) {
    ...
    if (split.length === 1) {
      ...
      if (c.length === 6) {
        hex =
          (hexStr.indexOf(c[0]) << 4) +
          hexStr.indexOf(c[1]) +
          (hexStr.indexOf(c[2]) << 12) +
          (hexStr.indexOf(c[3]) << 8) +
          (hexStr.indexOf(c[4]) << 20) +
          (hexStr.indexOf(c[5]) << 16);
      } else if (c.length === 3) {
        hex =
          hexStr.indexOf(c[0]) * 17 +
          hexStr.indexOf(c[1]) * 4352 +
          hexStr.indexOf(c[2]) * 1114112; // These may seem like magic numbers, but it's just converting #1bc, for example, into #11bbcc.
      }
      if (!isFinite(hex)) {
        return false;
      }
...
function saveLocation() {
  ...
  if (juliaMode) {
    str =
      ...
      zoom * w +
      " Type: " +
      fractalNames[fractalType] +
      " Julia Shading: " +
      shadingNames[shadingEffect] +
      " Julia X: " +
      juliaX +
      " Julia Y: " +
      juliaY;
