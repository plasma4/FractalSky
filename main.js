...
function oklab_to_srgb(oklab_array) {
    ...
    var [r_linear, g_linear, b_linear] = multiplyMatrixVector(M1_INVERSE_LMS_TO_LINEAR_SRGB, lms_linear)
...
function setupWebWorkers(amount) {
    for (let i = 0; i < amount; i++) {
        ...
        try {
            ...
            help.style.display = "unset"
            help.innerHTML = 'Unfortunately, an error occured while creating the memory. This may be because of memory limitations; try adding ?maxMB=4096 to the start of the URL (no commas in the number), or check the console if that doesn\'t work. Press Close and export and copy the current fractal position if needed.<br><button onclick="help.removeAttribute(\'style\')" id="infoClose">Close</button>'
            throw e
        }
